...
public:
    ...
    virtual bool contains(T vertex)
    {
        ...
        return this->nodeList.contains(vertexNode);
    }
    virtual float weight(T from, T to)
    {
        ...
        {
            throw EdgeNotFoundException(this->edge2Str());
        }

        return fromNode->getEdge(toNode)->weight(toNode);
    }
    ...
    virtual DLinkedList<T> getInwardEdges(T to)
    {
        ...
        DLinkedList<VertexNode *> list;

        typename DLinkedList<VertexNode *>::Iterator nodeIt = nodeList.begin();
        while (nodeIt != nodeList.end())
        {
            VertexNode *node = *nodeIt;
            typename DLinkedList<VertexNode *>::Iterator edgeIt = node->adList.begin();
            while (edgeIt != nodeList.end())
            {
                Edge *edge = *edgeIt;
                if (edge->to == toNode)
                {
    ...
    virtual int inDegree(T vertex)
    {
        ...
        VertexNode *vertexNode = this->getVertexNode(vertex);
        if (!vertexNode)
            throw VertexNotFoundException(this->vertex2Str(vertexNode));
...
public:
    ...
    public:
        ...
        {
            // TODO
            Edge *existEdge = this->getEdge(to);
            if (existEdge)
            {
        ...
        {
            // TODO
            int foundIndex = this->adList.indexOf(to);
            if (foundIndex == -1)
                return nullptr;
            return this->adList.get(foundIndex);
        }
        ...
        bool equals(VertexNode *node)
        {
            // TODO
            if (this->vertexEQ)
                return this->vertexEQ(this, node);
        ...
        void removeTo(VertexNode *to)
        {
            ...
            this->adList.removeAt(foundIndex);
        }
        int inDegree()
        {
            // TODO
